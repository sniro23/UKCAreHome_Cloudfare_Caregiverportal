...
const MyRequestsTab = () => {
    const { shiftRequests, isLoadingRequests, fetchShiftRequests } = useRosterStore(state => ({
        shiftRequests: state.shiftRequests,
        isLoadingRequests: state.isLoadingRequests,
        fetchShiftRequests: state.fetchShiftRequests,
    }));
    useEffect(() => {
        fetchShiftRequests();
    }, [fetchShiftRequests]);
...
export function RosterPage() {
  ...
  const { shifts, isLoading, fetchShifts, submitShiftRequest } = useRosterStore(state => ({
    shifts: state.shifts,
    isLoading: state.isLoading,
    fetchShifts: state.fetchShifts,
    submitShiftRequest: state.submitShiftRequest,
  }));
  useEffect(() => {
    fetchShifts();
  }, [fetchShifts]);
